...
export default function CommentSection({
  ...
  const deleteComment = async (id: string, userId: string) => {
    ...
    const res = isAdmin && userId !== currentUserId
      ? await fetch("/api/admin", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ action: "delete_comment", target_id: id }),
        })
          ...
          return (
            <div key={c.id} style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.5 }}>{c.content}</p>
              </div>
      ...
      {currentUserId ? (
        <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
          <textarea
            ...
            style={{ flex: 1, padding: "8px 10px", borderRadius: 3, resize: "vertical", minHeight: 56, fontSize: 13 }}
          />
          <button
            onClick={submit}
