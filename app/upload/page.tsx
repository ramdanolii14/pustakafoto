...
export default function UploadPage() {
  ...
  const addFiles = (incoming: File[]) => {
    ...
    setFiles((prev) => [...prev, ...valid.map((f) => ({
      file: f, preview: URL.createObjectURL(f), status: "pending" as const,
    }))]);
  };
  ...
  const toggleNude = () => {
    const next = !isNude;
    setIsNude(next);
    if (next) setIsMembersOnly(true); // nude implies members only
  };
  ...
  // ── Styles (after hooks, before render) ──
  ...
  // Show loading while session resolves — but DON'T return early that would reset state
  if (isPending) {
    return (
      <AppShell>
        ...
        <div style={{ display: "grid", gap: 12 }}>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              ...
              <div>
                <label style={labelStyle}>
                  <span style={{ display: "flex", alignItems: "center", gap: 5 }}><Tag size={11} /> Tags</span>
                </label>
          ...
          <div style={{ background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: 3, padding: "14px 18px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {/* Members only toggle */}
              <button type="button" onClick={() => setIsMembersOnly(!isMembersOnly)} style={toggleBtn(isMembersOnly)}>
          ...
          {files.length > 0 && (
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: 5 }}>
                {files.map((f, i) => (
                  <div key={i} onClick={() => !submitting && setThumbnailIndex(i)}
                    style={{ position: "relative", aspectRatio: "3/4", borderRadius: 2, overflow: "hidden", border: `2px solid ${i === thumbnailIndex ? "var(--accent)" : "var(--border)"}`, cursor: submitting ? "default" : "pointer", background: "var(--bg-3)" }}>
                    <img src={f.preview} alt={f.file.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    {i === thumbnailIndex && (
                      <div style={{ position: "absolute", top: 4, left: 4, background: "var(--accent)", color: "#000", fontSize: 9, fontWeight: "bold", padding: "1px 5px", borderRadius: 2 }}>THUMB</div>
                    )}
                    {f.status === "uploading" && (
                    ...
                    {f.status === "error" && (
                      <div style={{ position: "absolute", inset: 0, background: "rgba(204,68,68,0.3)", display: "flex", alignItems: "center", justifyContent: "center" }}>
